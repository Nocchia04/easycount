...
const Settings = () =>  {
  ...
  const deleteParam = (key) => {
    ...
    axios.post('https://easycount-8a1d6b5ada49.herokuapp.com/inputs/delete_input/', formdata).then((response) => {
      if(response.data.status == 'success'){
        const formdata = new FormData()
        formdata.append('id', id)
        axios.post('https://easycount-8a1d6b5ada49.herokuapp.com/inputs/get_inputs/', formdata).then((response) => {
    ...
    <div className='settings-root-container'>
      ...
      <div className='settings-display-container'>
        <div className='personalized-title-container'>
          <p className='personalized-title'>Crea e aggiungi le formule per controllare<br></br> la tua attività come vuoi tu! <span className='scopri-come' onClick={() => startTutorial()}>Scopri come!</span></p>
        </div>
